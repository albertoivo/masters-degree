...
Trabalhos recentes têm demonstrado a aplicabilidade de técnicas de \textit{machine learning} para classificação e segmentação de estruturas em imagens airglow, porém lacunas importantes permanecem em aberto \cite{Githio2024}: (i) não existe um modelo otimizado especificamente para o contexto das imagens EMBRACE/INPE com suas características instrumentais particulares; (ii) a performance e acurácia de diferentes modelos de ML não foi sistematicamente comparada para este problema específico; (iii) estratégias de aumento de dados (\textit{data augmentation}) carecem de investigação aprofundada neste domínio; e (iv) a integração de modelos de detecção em sistemas operacionais de tempo real ainda representa um desafio técnico.

Diante desse contexto, este projeto busca responder às seguintes questões de pesquisa:

\begin{enumerate}
\item Qual modelo de ML apresenta melhor desempenho na detecção automática de bolhas de plasma equatorial em imagens airglow 630,0 nm do programa EMBRACE/INPE?
...
\item É viável implementar um sistema de detecção automática em tempo real?
\end{enumerate}