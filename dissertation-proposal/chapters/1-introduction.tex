...
As Bolhas de Plasma Equatorial (EPB - sigla em inglês referente à \textit{Equatorial Plasma Bubbles}) são regiões caracterizadas por baixa densidade eletrônica, com ocorrência predominante entre os meses de setembro e março, logo após o pôr do sol \cite{Barros2018}. Elas submetem os sinais de rádio a variabilidades de amplitude e de fase, afetando, por exemplo, o funcionamento de sistemas tecnológicos que utilizam os sinais dos Sistemas Globais de Navegação por Satélite (GNSS) para navegação \cite{Githio2024}.
...
Técnicas de aprendizado de máquina têm sido aplicadas para automação da detecção de EPBs. \citeonline{Thanakulketsarat2023} desenvolveram um modelo combinado de Redes Neurais Convolucionais (CNN sigla em inglês para \textit{Convolutional Neural Network}) e Máquina de Vetores de Suporte (SVM - sigla em inglês para \textit{Support Vector Machine}) com kernel de Função de Base Radial (RBF - sigla em inglês para \textit{Radial Basis Function}), alcançando acurácia de 93,67\%. \citeonline{Yacoub2025} propuseram uma abordagem utilizando Análise de Componentes Principais Bidimensional (2DPCA - sigla em inglês para \textit{Two-Dimensional Principal Component Analysis}) para extração de características espaciais, combinada com \textit{Recursive Feature Elimination} (RFE) e classificador Random Forest, atingindo acurácia de 98,17\%. Embora promissores, estes estudos focaram principalmente em datasets específicos, havendo necessidade de adaptação às características particulares das imagens airglow brasileiras do programa EMBRACE/INPE.

Este projeto propõe o desenvolvimento de uma abordagem baseada em \textit{machine learning} para detecção automática de EPBs em imagens airglow do EMBRACE, explorando arquiteturas modernas adaptadas às características específicas dessas estruturas ionosféricas. A pesquisa situa-se na interseção entre ciências espaciais e inteligência artificial, contribuindo tanto para o avanço metodológico em visão computacional quanto para o aprimoramento dos sistemas operacionais de monitoramento de clima espacial no Brasil. Espera-se reduzir o tempo de detecção, aumentar a consistência histórica e fornecer base metodológica para aplicações preditivas subsequentes do clima espacial no Brasil.
